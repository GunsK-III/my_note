...
print(file_list)
clf = joblib.load(path_models+'clf_gboost.m')   # 加载模型
width_compressed, height_compressed = 360, 640
cap = cv2.VideoCapture(r"D:\NewFolder\ObjectDetection_YF\videos\test.mp4")
...
while cap.isOpened():
    ...
    if ret:
        ...
        frame0 = cv2.resize(frame0, (width_compressed, height_compressed))
        ...
        # 确保tun_left_w和tun_top_h已定义再使用
        if 'tun_left_w' in locals() and 'tun_top_h' in locals():
            ...
            print("无法绘制矩形，因为未检测到人区域的边界坐标。")

        cv2.imshow('my_video_YF46', frame)
        if count % 10 == 0:
            current = time.time()
